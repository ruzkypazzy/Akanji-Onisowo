...
✅ **Live bot** (public, no login required to view):
   ...
   - The bot is **locked to the owner's Telegram user ID for trading**.
     Real-trade commands (`/pick`, `/buy`, `/sell`, `/pickfuture`,
     `/pickspot`, `/autotrade`, `/close`, etc.) only work for the
     owner. Everyone else sees demo mode.
   - **All trades use the owner's Bitget account** (UID 7781181263).
     The bot's `OWNER_TELEGRAM_ID` and `BITGET_API_KEY` are both
     tied to the same person. So a "locked bot" means: only the
     owner can request trades, and all trades execute on the owner's
     Bitget account regardless.
   - Anyone can run these read-only commands in the bot:
     ...
     - `/analyze BTCUSDT 1` — **live agent loop**. Real Qwen, real
       market data, real 17+ skill chain. No real order is placed.
   - To trade in real time, install your own copy: it's open source.
     ```bash
     bash <(curl -sL https://raw.githubusercontent.com/ruzkypazzy/Akanji-Onisowo/main/install.sh) --user
     ```
     5 minutes. Your keys, your VPS, your trades. The installer
     prompts for `OWNER_TELEGRAM_ID` so only you can run real-trade
     commands.
