...
The bot at [@OnisowoBot](https://t.me/OnisowoBot) is **locked to the owner's Telegram user ID**. So you can't trade on their account. To see what it does, type these commands in Telegram — they are read-only, no real money:
...
**All trades use the owner's Bitget account** (UID 7781181263). The bot's `OWNER_TELEGRAM_ID` and `BITGET_API_KEY` are both tied to the same person — only the owner can request trades, and every trade executes on the owner's Bitget account regardless.

If you want to use Àkànjí in real time, install your own copy — it's open source:

```bash
bash <(curl -sL https://raw.githubusercontent.com/ruzkypazzy/Akanji-Onisowo/main/install.sh) --user
```

5 minutes. Your keys, your VPS, your trades. The installer prompts for `OWNER_TELEGRAM_ID` so only you can run real-trade commands.
